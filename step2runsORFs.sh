...
##sORF from ab initio
	##extract igr
		...
		perl ../myigrgff.pl "$lines"_gene.gff "$lines"_genomic.gff > "$lines"_igr.gff
	...
	##predict sORF by sorffinder
		##makemodel	
		perl pathtosORFinder/src/make_model.pl -c *coding.fasta -n *intron.fasta -o "$lines"_matrix
		##simulate
		perl pathtosORFinder/src/simulate.pl -m *_matrix -p 0.5 -o "$lines"_simulate0.5
		##sorffinder
		#perl pathtosORFinder/src/search_sORF.pl -m *_matrix -p 0.5 -s *_simulate0.5 -i "$lines"_igr.fasta -o "$lines"_sORF0.5.fasta -d b
	...
	##rename fasta header according to software used
		...
		blastx -db *_protein.faa -query "$lines"_sORF_predicted.fasta -out "$lines"_sORF_abinitiopredictedVSprotein -outfmt 6 -evalue 0.00009 -num_threads 5 -word_size 2 -gapopen 9 -gapextend 1 -matrix PAM30

		##grep sorf seq hit protein
		cut -f1 "$lines"_sORF_abinitiopredictedVSprotein | sort -u | sed 's/\_1//g'> abinitio_sorf_hits_protein
...
##create file for selected cluster	
	...
	cd cluster_sORF_combined_cdhit70_ori
		...
		find . -type f | wc -l
	cd ..
	echo "mv conserved clusters into folder cluster_listuniq_70"
